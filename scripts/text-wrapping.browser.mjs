...
export async function inspectTextWrapping(targets = []) {
  ...
  const inspectHeadingTextNode = (node, heading, label) => {
  ...
  const inspectHeading = (heading) => {
    ...
    for (const node of nodes) inspectHeadingTextNode(node, heading, label);
  ...
  const inspectLink = (link) => {
    ...
    const annotatedAsRaw = link.dataset.rawUrl !== undefined;
    const style = getComputedStyle(link);
    if (annotatedAsRaw !== raw) errors.push(`incorrect URL annotation: ${link.textContent}`);
    ...
    for (const node of textNodes(link)) {
      for (const rect of range(node).getClientRects()) within(rect, box(parent), 'raw URL');
    }
  };

  const inspectScrollable = (element) => {
  ...
  targets.forEach(requireHeading);
  headings.filter(visible).forEach(inspectHeading);
  splitWords.forEach(inspectSplitWord);
  document.querySelectorAll('.page-head p, .editorial-head p, .tool-card p, .prose p, .prose li').forEach(inspectParagraph);
  document.querySelectorAll('.result-panel, .reference-grid, .reference-grid > section').forEach(inspectContainer);
  const links = [...document.querySelectorAll('.prose a')];
  links.forEach(inspectLink);
  document.querySelectorAll('.prose table, .prose .katex-display').forEach(inspectScrollable);
  document.querySelectorAll('code, kbd, samp, var, .katex').forEach(inspectTechnicalText);
  document.querySelectorAll('.hit-desc').forEach(inspectHitDescription);
  return { route: location.pathname, width: innerWidth, lang: document.documentElement.lang, errors, splitWords, boxes, headings: headings.length, rawUrls: links.filter((link) => link.dataset.rawUrl !== undefined).length, ordinaryLinks: links.filter((link) => link.dataset.rawUrl === undefined).length };
}

async function discoverBuiltRoutes(root, parse) {
  const routes = [];
...
async function runWrappingSweep(adapter, routes, widths) {
  let combinations = 0;
  let headings = 0;
  let rawUrls = 0;
  let ordinaryLinks = 0;
  ...
  return { combinations, headings, rawUrls, ordinaryLinks };
}
...
async function run() {
  ...
  const routes = await discoverBuiltRoutes(root, parse);
  ...
  try {
    ...
    send = (method, params = {}, sessionId = undefined) => new Promise((resolve, reject) => {
    ...
    const { combinations, headings, rawUrls, ordinaryLinks } = await runWrappingSweep(adapter, routes, widths);
